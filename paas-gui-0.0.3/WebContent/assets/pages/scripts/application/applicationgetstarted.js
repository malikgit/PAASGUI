...
mycloudprovider.controller('MainCtrl', function ($scope,$http) {
   ...
   //Upload Application XML File
   $scope.uploadApplicationFile = function(){
	 	  //document.getElementById('importFile').click();
	       var file = document.getElementById('importFile').files[0],
	       reader = new FileReader();
	       reader.onloadend = function(e){
	     	  $scope.data = e.target.result;
	     	  var res = $http.post('/paas-gui/rest/applicationService/addApplicationXml', $scope.data);
	     	  res.success(function(data){
	     		  if(angular.equals(data, "Error Code: 404, Subnet Name Doesn't Exist")) {
	     		  	$scope.validateXML = data;
	     		  }
	     	  });
	       };
	       reader.readAsBinaryString(file);
	     };//End of uploadApplicationFile method

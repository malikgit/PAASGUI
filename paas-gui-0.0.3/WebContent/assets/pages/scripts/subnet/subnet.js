...
app.controller('SubnetCtrl', function($scope, srvShareData, $location,$http, $window) {
  ...
  //Upload Subnet XMl File
  $scope.uploadSubnetXmlFile = function(){
	  //document.getElementById('importFile').click();
      var file = document.getElementById('importFile').files[0],
      reader = new FileReader();
      reader.onloadend = function(e){
    	  $scope.data = e.target.result;
    	  var res = $http.post('/paas-gui/rest/subnetService/addXmlSubnet', $scope.data);
    	  res.success(function(data, status, headers, config) {
    		  $scope.selectSubnetnew();
    	  });
    	  res.error(function(data, status, headers, config) {
    		  alert("Error in Subnet XML");
    	  });
    	  
      };
      reader.readAsBinaryString(file);
    };//End of uploadSubnetXmlFile method
    
  //Download Subnet XML File
    $scope.exportXmlSubnet = function() {
   	 var response = $http.get('/paas-gui/rest/subnetService/exportAllSubnet');
   	 response.success(function(data){
		 ...
		 element.href = url;
		 element.setAttribute('download', 'subnet.xml');

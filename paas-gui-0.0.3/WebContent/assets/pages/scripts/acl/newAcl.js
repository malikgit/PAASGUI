...
app.controller('MainCtrl', function($scope, srvShareData, $location,$http) {
 ...
 $scope.deleteAcl = function(data) {
  ...
  //upload acl xml file
  $scope.uploadAclXMLFile = function(){
	  //document.getElementById('importFile').click();
      var file = document.getElementById('importFile').files[0],
      reader = new FileReader();
      reader.onloadend = function(e){
    	  $scope.data = e.target.result;
    	  var res = $http.post('/paas-gui/rest/aclService/addXMLACLRule', $scope.data);
    ...
    };//End of uploadAclFile method
    
    //Download VPC XML File
    $scope.exportXmlVpc = function() {
   	 var response = $http.get('/paas-gui/rest/aclService/getAllACLInXMLFormat');
   	 response.success(function(data){

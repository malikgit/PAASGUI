...
public class VPCJAXBHelper {

	private static final Logger logger = LoggerFactory.getLogger(VPCJAXBHelper.class);
	private static final String W3C_XML_SCHEMA_NS_URI = "http://www.w3.org/2001/XMLSchema";

	/**
	 * this method is used to validate the xml
	 * @param configName
	 * @param schemaName
	 * @return
	 */
	...
	/**
	 * this method is used to get vpc configuration
	 * @param vpcXml
	 * @param schemaName
	 * @return
	 * @throws UnableToUnmarshalVPC
	 */
	public TenantConfiguration getVPCConfiguration(String vpcXml, String schemaName) throws UnableToUnmarshalVPC {
		try {
			JAXBContext context = JAXBContext.newInstance(TenantConfiguration.class);
			Unmarshaller jaxbUnmarshaller = context.createUnmarshaller();
			boolean validateXml = validateConfig(vpcXml, schemaName);
			logger.debug("Validation : " + validateXml);
			if(validateXml) {
				InputStream isConfig = new ByteArrayInputStream(vpcXml.getBytes());
				Source configSource = new StreamSource(isConfig);
				return (TenantConfiguration) jaxbUnmarshaller.unmarshal(configSource);
			} else {
				throw new UnableToUnmarshalVPC("Unable to Unmarshal VPC");
			}
